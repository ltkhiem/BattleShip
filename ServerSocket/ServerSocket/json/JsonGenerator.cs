            ...
            foreach (var item in listObject)
            {
                ...
                else
                {
                    if (kvPair.Value is string || kvPair.Value is String)
                        result += $"\"{ValidateString(kvPair.Value.ToString())}\"";
                    else if (kvPair.Value is IDictionary)
                        result += $"{GenerateJsonObject((Dictionary<string, object>)kvPair.Value)}";
                    else if (kvPair.Value is IList)
                        result += $"{GenerateJsonArray((IList)kvPair.Value)}";
                    else
                        result += $"{kvPair.Value}";
                }
            ...
            for (int i = 0; i < listObject.Count; i++)
            {
                ...
                else
                {
                    if (currentObject is string || currentObject is String)
                        result += $"\"{ValidateString(currentObject.ToString())}\"";
                    else if (currentObject is IDictionary)
                        result += $"{GenerateJsonObject((Dictionary<string, object>)currentObject)}";
                    else if (currentObject is IList)
                        result += $"{GenerateJsonArray((List<object>)currentObject)}";
                    else
